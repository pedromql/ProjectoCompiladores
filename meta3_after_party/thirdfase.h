...
void varDecl_funtion(Node * , Table_structure *, Table_structure * first_table);
void funcPart_function(Node * , Table_structure  *, Table_structure * first_table);
...
void check_duplicate(Table_structure *, char * , int, int);
void check_var_type(Table_structure * , Node * , int , int);
int search_type_in_tables(Table_structure * , char *);
void check_var_type_funcdef2(Table_structure *, Table_structure * , Node * , int , int );
int search_type_in_tables_funcdef2(Table_structure * , Table_structure * , char * );
void check_statements(Node *, Table_structure *);
void check_assign(Node * , Table_structure * );
void search_var_in_table(Node * , Table_structure * );
...
void ast(Node * root, Table_structure * first_table, Table_structure * last_table) {
    if (root != NULL) {
        if (strcmp(root->type, "VarDecl") == 0) {
            varDecl_funtion(root, last_table, first_table);
        }
        else if (strcmp(root->type, "FuncPart") == 0) {
            funcPart_function(root,last_table, first_table);
...
void varDecl_funtion(Node * parent, Table_structure *last_table, Table_structure * first_table) {
    ...
    while(var_nodes->brother != NULL) { //iterate through all variables
        if (last_table->data == NULL) { //if last_table has no data yet, a first line is created
            ...
            //needless to search duplicate when it's a first line

            check_var_type(first_table, var_nodes, var_nodes->line, var_nodes->col);
            ...
            check_duplicate(last_table, lower_case(var_nodes->value), var_nodes->line, var_nodes->col);
            check_var_type(first_table, var_nodes, var_nodes->line, var_nodes->col);
...
void funcPart_function(Node * parent, Table_structure  *last_table, Table_structure * first_table) {
    ...
    while(temp != NULL) { //iterate through all funcPart sons (funcdef, funcdef2, funcdecl) and add it's type to the program symbol table
        if (strcmp(temp->type,"FuncDef2") == 0);
            ...
            check_var_type(first_table, temp->son, temp->son->line, temp->son->col);

            last_table->data = first_line(lower_case(temp->son->value), type[type_function], NULL, NULL);
        }
        else {

            check_duplicate(last_table, lower_case(temp->son->value), temp->son->line, temp->son->col);
            check_var_type(first_table, temp->son, temp->son->line, temp->son->col);
...
void funcDef_function(Node * parent, Table_structure * first_table) {
    ...
    check_var_type(first_table, parent->son, parent->son->line, parent->son->col);
        ...
        while(too_many_params != NULL) {
            ...
            if (too_many_params->son != NULL) { //if function has parameters
                ...
                while(temp->brother != NULL){ //add all function parameters to the table with it's name and type
                    ...
                    check_duplicate(new_table, lower_case(temp->value), temp->line, temp->col);
                    check_var_type(first_table, temp, temp->line, temp->col);
    ...
    if (parent->son->brother->brother->brother->brother != NULL) check_statements(parent->son->brother->brother->brother->brother, first_table);
}
...
void funcDef2_function(Node * parent, Table_structure * first_table) { //Falta verificar se a funcao já existe em tabela
    ...
    while (temp != NULL) {
        ...
        while (var_nodes->brother != NULL) { //iterates through all variables and adds them to the the function table
            ...
            check_duplicate(find_table, lower_case(var_nodes->value), var_nodes->line, var_nodes->col);
            check_var_type_funcdef2(first_table, find_table, var_nodes, var_nodes->line, var_nodes->col);
...
void check_duplicate(Table_structure *table, char * value, int line, int col) {
    Table_lines * lines = table->data;

    while (lines != NULL) {
        if (strcmp(lines->name,value) == 0) {
            error_symbolalareadydefined(value,line,col);
            exit(0);
        }
        lines = lines->next;
    }
...
void check_var_type(Table_structure * first_table, Node * first_node, int line, int col) {
    ...
    if (strcmp(first_node->brother->type,"Id") != 0 ) temp = first_node->brother->brother;
    
    error = search_type_in_tables(first_table, temp->value);

    if (error == 1) {
        error_typeidentifierexpected(temp->line,temp->col);
        exit(0);
    }
    else if (error == 2) {
        error_symbolnotdefined(temp->value, temp->line, temp->col);
        exit(0);
    }
...
int search_type_in_tables(Table_structure * first_table, char * type) {
    Table_lines * global_vars;
    Table_structure * last_table;
    Table_lines * last_table_lines;
    //"search" first table
    if (strcmp(lower_case(type),"integer") == 0) return 0;
    else if (strcmp(lower_case(type),"real") == 0) return 0;
    else if (strcmp(lower_case(type),"boolean") == 0) return 0;

    //search global vars
    if (first_table->next->next->data != NULL) {
        global_vars = first_table->next->next->data;
        while (global_vars != NULL) {
            if (strcmp(lower_case(type),global_vars->name) == 0) return 1;
            global_vars = global_vars->next;
        }
    }

    if (first_table->next->next->next != NULL) {
        last_table = first_table->next->next->next;
        while (last_table->next != NULL) { //get to last table
            last_table = last_table->next;
        }
        last_table_lines = last_table->data;
        while (last_table_lines != NULL) {
            if (strcmp(lower_case(type),last_table_lines->name) == 0) return 1;
            last_table_lines = last_table_lines->next;
        }
...
void check_var_type_funcdef2(Table_structure *first_table, Table_structure * funcdef2_table, Node * first_node, int line, int col) {
    Node * temp = first_node;
    int error;
    ...
    error = search_type_in_tables_funcdef2(first_table, funcdef2_table, temp->value);
...
int search_type_in_tables_funcdef2(Table_structure * first_table, Table_structure * fundef2_table, char * type) {
    Table_lines * global_vars;
    Table_structure * last_table;
    Table_lines * last_table_lines;
    //"search" first table
...
void check_statements(Node * node, Table_structure * table) {
    Node * temp = node;

    if (temp == NULL) return;
    if (strcmp(temp->type,"Assign") == 0) {
        check_assign(temp,table);
    }

    
    check_statements(node->son, table);
    check_statements(node->brother, table);

}

void check_assign(Node * node, Table_structure * first_table) {
    Node * temp = node;
    Table_structure * tables = first_table;

    if (strcmp(temp->son->type,"Id") == 0) search_var_in_table(temp->son,tables);
    if (strcmp(temp->son->brother->type,"Id") == 0) search_var_in_table(temp->son->brother,tables);



}

void search_var_in_table(Node * node, Table_structure * first_table) {
    Table_lines * lines = first_table->next->next->data;
    TableStructure * tables = first_table;
    char * var = node->value;
    if (strcmp(var,"integer") == 0) return;
    else if (strcmp(var,"real") == 0) return;
    else if (strcmp(var,"boolean") == 0) return;
    //printf("%s\n",var);
    //search in global vars
    while (lines != NULL) {
        if (strcmp(lower_case(var),lines->name) == 0) return;
        lines = lines->next;
    }
    ...
    error_symbolnotdefined(var,node->line,node->col);
    exit(0);
}
